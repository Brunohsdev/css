...
function gerenciarFocoModal(modalId){
    ...
    const elementosModal = modal.querySelectorAll('a, button, input, textarea, select, [tabindex]:not([tabindex="-1"])');
    ...
    modal.addEventListener("keydown", (event)=>{
        if(event.key === "Tab"){
            if(event.shiftKey){
                //se a tecla shift + tab for pressionada e o foco estiver no primeiro elemento, mover para o ultimo elemento
                if(document.activeElement === primeiroElemento){
                    event.preventDefault();
                    ultimoElemento.focus();
            }
        }else{
            //se a tecla tab for pressionada e o foco estiver no ultimo elemento, mover para o primeiro elemento
